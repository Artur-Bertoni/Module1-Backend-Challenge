...
public class HeaderColumnNameAndOrderMappingStrategyTest<T> extends HeaderColumnNameMappingStrategy<T> {

    public HeaderColumnNameAndOrderMappingStrategyTest(Class<T> type) {
        setType(type);
    }
    ...
    protected CsvConverter determineConverter(Field field, Class<?> elementType, String locale,
        ...
        if(customConverter != null && !customConverter.equals(AbstractCsvConverter.class)) {
            return super.determineConverter(field, elementType, locale, null, customConverter);
        }
